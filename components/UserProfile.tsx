...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!snap.exists()) {
          ...
          // ✅ PROTECTION: শুধু নতুন user-ই refer হতে পারবে (existing user এ pendingReferral তৈরি হবে না)
          // nu তে referredBy নেই মানে এই user আগে কখনো refer হয়নি — safe to proceed
  ...
  const referralLink = React.useMemo(() => {
    ...
    if (!cs.referralBotUsername) return '';
    const app = (cs.referralAppName || '').trim();
    return app
      ? `https://t.me/${cs.referralBotUsername}/${app}?startapp=ref_${userData.referralCode}`
      : `https://t.me/${cs.referralBotUsername}?start=ref_${userData.referralCode}`;
  }, [cs.referralBotUsername, cs.referralAppName, userData?.referralCode]);
